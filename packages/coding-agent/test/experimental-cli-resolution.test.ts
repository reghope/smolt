...
describe("experimental CLI command composition", () => {
	test("composes smolt command options with the existing parser", () => {
	...
	test.each(["--help", "--version"] as const)("keeps Smolt %s handling in existing CLI options", (option) => {
		expect(experimentalCli.parse([option])).toMatchObject({
			ok: true,
			command: { command: "smolt", options: { [option === "--help" ? "help" : "version"]: true } },
		});
	});
	...
	test.each(["smolt", "server", "client"] as const)("executes the parsed %s command", async (name) => {
		const context = {
			runSmolt: vi.fn(() => undefined),
			runServer: vi.fn(() => undefined),
			runClient: vi.fn(() => undefined),
		};
		const result = await experimentalCli.execute(name === "smolt" ? [] : [name], context);

		expect(result).toMatchObject({ ok: true, command: { command: name } });
		expect(context.runSmolt).toHaveBeenCalledTimes(name === "smolt" ? 1 : 0);
