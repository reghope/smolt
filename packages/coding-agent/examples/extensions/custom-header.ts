...
function getSmoltMascot(theme: Theme): string[] {
	// --- COLORS ---
	// 3b1b Blue: R=80, G=180, B=230
	const smoltBlue = (text: string) => theme.fg("accent", text);
	...
	const lineBar = `  ${smoltBlue(BLOCK.repeat(14))}`;

	// 4. Lines 3-6: The Legs
	// Indented 5 spaces relative to the very left edge
	// Leg width: 2 blocks | Gap: 4 blocks
	const lineLeg = `     ${smoltBlue(BLOCK.repeat(2))}    ${smoltBlue(BLOCK.repeat(2))}`;
...
export default function (smolt: ExtensionAPI) {
			...
			ctx.ui.setHeader((_tui, theme) => {
				return {
					render(_width: number): string[] {
						const mascotLines = getSmoltMascot(theme);
	...
	smolt.registerCommand("builtin-header", {
