...
export interface ExtensionUIContext {
	 ...
	 * extend `CustomEditor` from `@smolt/coding-agent` and call
	 * `super.handleInput(data)` for keys you don't handle.
	 *
	 * @example
	 * ```ts
	 * import { CustomEditor } from "@smolt/coding-agent";
	 *
...
export interface ExtensionContext {
	...
	/** Gracefully shutdown smolt and exit. Available in all contexts. */
	shutdown(): void;
...
export interface BeforeAgentStartEvent {
	...
	systemPrompt: string;
	/** Structured options used to build the system prompt. Extensions can inspect this to understand what Smolt loaded without re-discovering resources. */
...
/** Fired when Smolt is no longer waiting on a blocking user-facing extension UI prompt. */
...
export interface ExtensionAPI {
	...
	/** Register a transformer for user and assistant Markdown before Smolt renders it in the interactive transcript. */
...
/** Configuration for registering a provider via smolt.registerProvider(). */
...
export type ExtensionFactory = (smolt: ExtensionAPI) => void | Promise<void>;
...
export interface ExtensionRuntimeState {
	...
	/** Native smolt-ai provider registrations queued during extension loading, processed when runner binds. */
...
/**
 * Action implementations for smolt.* API methods.
