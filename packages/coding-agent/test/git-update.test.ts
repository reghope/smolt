...
describe("DefaultPackageManager git update", () => {
	...
	describe("temporary git sources", () => {
		it("should refresh cached temporary git sources when resolving", async () => {
			...
			const extensionFile = join(cachedDir, "smolt-extensions", "session-breakdown.ts");

			rmSync(cachedDir, { recursive: true, force: true });
			mkdirSync(join(cachedDir, "smolt-extensions"), { recursive: true });
			writeFileSync(
				join(cachedDir, "package.json"),
				JSON.stringify({ smolt: { extensions: ["./smolt-extensions"] } }, null, 2),
			);
			...
			expect(getFileContent(cachedDir, "smolt-extensions/session-breakdown.ts")).toBe("// fresh");
		});
	});
});
