...
import { streamSimple as streamSmoltMessages } from "../src/api/smolt-messages.ts";
...
describe("fetch stream option", () => {
	...
	it("uses fetch for Mistral, Codex SSE, and smolt-messages HTTP requests", async () => {
		const { custom, fallback } = mockFetches();
		...
		await streamSmoltMessages(createModel("smolt-messages"), context, {
			apiKey: "test-key",
			fetch: custom,
		}).result();
