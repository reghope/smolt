...
```
~/.smolt/agent/sessions/--<path>--/<timestamp>_<uuid>.jsonl
```
...
Source on GitHub ([smolt-mono](https://github.com/earendil-works/pi-mono)):
...
For TypeScript definitions in your project, inspect `node_modules/@smolt/coding-agent/dist/` and `node_modules/@smolt/ai/dist/`.
...
### Base Message Types (from smolt-ai)
...
- `fromHook`: `true` if generated by an extension, `false`/`undefined` if smolt-generated (legacy field name)
...
Use `customType` to identify your extension's entries on reload. Interactive mode can render custom entries via `smolt.registerEntryRenderer(customType, renderer)`, but they still do not participate in LLM context.
...
Session metadata (e.g., user-defined display name). Set via `/name`, `--name` / `-n`, or `smolt.setSessionName()` in extensions.
