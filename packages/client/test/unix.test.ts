...
import { ClientMessageDecoder, encodeServerMessage, PROTOCOL_VERSION, type ServerSnapshot } from "@smolt/protocol";
...
test("rejects invalid Unix transport options", () => {
	...
	expect(() => createUnixTransportFactory({ path: "/tmp/smolt.sock", maxPendingBytes: 0 })).toThrow(/positive/);
});
...
describe.runIf(process.platform !== "win32")("Unix-domain sockets", () => {
	test("PiClient exchanges fragmented framed messages over a real Unix socket", async () => {
		const directory = await mkdtemp(join(tmpdir(), "smolt-client-"));
		const socketPath = join(directory, "smolt.sock");
