...
describe("DefaultPackageManager", () => {
	...
	beforeEach(() => {
		previousOfflineEnv = process.env.SMOLT_OFFLINE;
		delete process.env.SMOLT_OFFLINE;
	...
	afterEach(() => {
		if (previousOfflineEnv === undefined) {
			...
			process.env.SMOLT_OFFLINE = previousOfflineEnv;
		}
		vi.restoreAllMocks();
		vi.unstubAllGlobals();
...
Content`,
		...
		it("should auto-discover root markdown skills from .smolt skill dirs", async () => {
...
Content`,
		...
		it("should resolve project paths relative to .smolt", async () => {
			const extDir = join(tempDir, ".smolt", "extensions");
			mkdirSync(extDir, { recursive: true });
...
Content`,
				...
				symlinkSync(sharedExtensionsDir, join(tempDir, ".smolt", "extensions"), "dir");
				symlinkSync(sharedSkillsDir, join(tempDir, ".smolt", "skills"), "dir");
				symlinkSync(sharedPromptsDir, join(tempDir, ".smolt", "prompts"), "dir");
				symlinkSync(sharedThemesDir, join(tempDir, ".smolt", "themes"), "dir");
		...
		it("should auto-discover project prompts with overrides", async () => {
			const promptsDir = join(tempDir, ".smolt", "prompts");
			mkdirSync(promptsDir, { recursive: true });
		...
		it("should resolve directory with package.json smolt.extensions in extensions setting", async () => {
			// Create a package with smolt.extensions in package.json
			...
			// Should find the extensions declared in package.json smolt.extensions
	...
	describe("auto-discovered skill metadata", () => {
		it("should use the agent dir as baseDir for user .smolt/agent skills", async () => {
			const skillPath = join(agentDir, "skills", "user-smolt", "SKILL.md");
			mkdirSync(join(agentDir, "skills", "user-smolt"), { recursive: true });
			writeFileSync(skillPath, "---\nname: user-smolt\ndescription: user smolt\n---\n");
		...
		it("should use the project .smolt dir as baseDir for project .smolt skills", async () => {
			const projectBaseDir = join(tempDir, ".smolt");
			const skillPath = join(projectBaseDir, "skills", "project-smolt", "SKILL.md");
			mkdirSync(join(projectBaseDir, "skills", "project-smolt"), { recursive: true });
			writeFileSync(skillPath, "---\nname: project-smolt\ndescription: project smolt\n---\n");
	...
	describe(".agents/skills auto-discovery", () => {
		...
		it("should keep ~/.agents/skills user-scoped when cwd is under home in a non-git directory", async () => {
			...
			try {
				...
				const localAgentDir = join(tempDir, ".smolt", "agent");
		...
		it("should dedupe user skill entries when ~/.smolt/agent/skills is a symlink to ~/.agents/skills", async () => {
			const previousHome = process.env.HOME;
			process.env.HOME = tempDir;
	...
	describe("ignore files", () => {
		...
		it("should not apply parent .gitignore to .smolt auto-discovery", async () => {
			writeFileSync(join(tempDir, ".gitignore"), ".smolt\n");

			const skillDir = join(tempDir, ".smolt", "skills", "auto-skill");
			mkdirSync(skillDir, { recursive: true });
	...
	describe("resolveExtensionSources", () => {
		...
		it("should handle directories with smolt manifest", async () => {
		...
		it("should keep smolt manifest entries with leading tilde package-relative", async () => {
	...
	describe("command spawning", () => {
		it("should preserve argv entries containing spaces", () => {
			...
			const valueWithSpace = "C:\\Users\\A B\\.smolt\\npm";
	...
	describe("npmCommand", () => {
		...
		it("should update git package dependencies with --omit=dev", async () => {
			const source = "git:github.com/user/repo";
			const targetDir = join(tempDir, ".smolt", "git", "github.com", "user", "repo");
			mkdirSync(targetDir, { recursive: true });
			writeFileSync(join(targetDir, "package.json"), JSON.stringify({ name: "repo", version: "1.0.0" }));
		...
		it("should install user npm packages into the smolt-managed npm root", async () => {
			settingsManager = SettingsManager.inMemory({
				npmCommand: ["pnpm"],
	...
	describe("temporary install paths", () => {
		it("should place temporary npm packages under the agent temp extension folder", () => {
			...
			expect(installPath.startsWith(join(tmpdir(), "smolt-extensions"))).toBe(false);
	...
	describe("settings source normalization", () => {
		...
		it("should store project local packages relative to .smolt settings base", () => {
			...
			const rel = relative(join(tempDir, ".smolt"), projectPkgDir);
			const expected = rel.startsWith(".") ? rel : `./${rel}`;
			expect(settings.packages?.[0]).toBe(expected);
		});
	...
	describe("pattern filtering in smolt manifest", () => {
	...
	describe("pattern filtering in package filters", () => {
		...
		it("should resolve autoload-disabled project package entries as deltas over global packages", async () => {
			const pkgDir = join(agentDir, "npm", "node_modules", "smolt-tools");
			mkdirSync(join(pkgDir, "extensions"), { recursive: true });
			writeFileSync(join(pkgDir, "package.json"), JSON.stringify({ name: "smolt-tools", version: "1.0.0" }));
			writeFileSync(join(pkgDir, "extensions", "foo.ts"), "export default function() {}");
			writeFileSync(join(pkgDir, "extensions", "bar.ts"), "export default function() {}");
			settingsManager.setPackages(["npm:smolt-tools"]);
			settingsManager.setProjectPackages([
				{ source: "npm:smolt-tools", autoload: false, extensions: ["-extensions/foo.ts"] },
			]);
			const runCommandSpy = vi
				.spyOn(packageManager as unknown as PackageManagerInternals, "runCommand")
		...
		it("should resolve autoload-disabled package entries as positive-only without a global package", async () => {
			...
			settingsManager.setProjectPackages([
				{ source: relative(join(tempDir, ".smolt"), pkgDir), autoload: false, extensions: ["+extensions/foo.ts"] },
			]);
...
export default function(api) { api.registerTool({ name: "test", description: "test", execute: async () => helper() }); }`,
		...
		it("should respect package.json smolt.extensions manifest in subdirectories", async () => {
	...
	describe("offline mode and network timeouts", () => {
		it("should update npm range packages using the configured spec", async () => {
			const installedPath = join(tempDir, ".smolt", "npm", "node_modules", "example");
			...
			expect(runCommandSpy).toHaveBeenCalledWith(
				"npm",
				["install", "example@^1.0.0", "--prefix", join(tempDir, ".smolt", "npm"), "--legacy-peer-deps"],
				undefined,
			);
		});
		...
		it("should batch npm updates per scope and run git updates in parallel while skipping pinned npm and current packages", async () => {
			...
			const projectOldPath = join(tempDir, ".smolt", "npm", "node_modules", "project-old");
			const projectCurrentPath = join(tempDir, ".smolt", "npm", "node_modules", "project-current");
		...
		it("should skip pinned packages when checking for updates", async () => {
			const installedNpmPath = join(tempDir, ".smolt", "npm", "node_modules", "example");
