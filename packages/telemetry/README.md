...
Applications can use the in-memory reference or provide an adapter for OpenTelemetry, Sentry, logs, or another backend. Smolt packages pass telemetry contexts explicitly and define their domain schemas separately.
...
- [Smolt Package Integration](#smolt-package-integration)
...
```typescript
import { NOOP_TELEMETRY_CONTEXT } from '@smolt/telemetry';
...
```typescript
import { InMemoryTelemetryContext } from '@smolt/telemetry';
...
`@smolt/telemetry/testing` exports a runner-independent conformance suite modeled as grouped cases. A fixture supplies a fresh context and converts its backend's finished spans into normalized `RecordedTelemetrySpan` snapshots:

```typescript
import {
  createTelemetryAdapterConformance,
  type TelemetryAdapterFixture,
} from '@smolt/telemetry/testing';
...
```typescript
import { AGENT_TELEMETRY_SCHEMAS } from '@smolt/agent-core';
