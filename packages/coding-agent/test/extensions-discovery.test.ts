...
describe("extensions discovery", () => {
	...
	beforeEach(() => {
		tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "smolt-ext-test-"));
	...
	it("loads the coding-agent entrypoint without rewriting smolt-ai provider subpaths", async () => {
		fs.writeFileSync(
	...
	it("keeps the type-only smolt-ai OAuth compatibility barrel resolvable", async () => {
		fs.writeFileSync(
			path.join(extensionsDir, "oauth-import.ts"),
			`
				import * as oauth from "@smolt/ai/oauth";
				void oauth;
				export default function(smolt) {
					smolt.registerCommand("test", { handler: async () => {} });
				}
	...
	it("discovers subdirectory with package.json smolt field", async () => {
	...
	it("keeps package.json smolt extension entries with leading tilde package-relative", async () => {
	...
	it("package.json with smolt field takes precedence over index.ts", async () => {
		const subdir = path.join(extensionsDir, "my-package");
		fs.mkdirSync(subdir);
	...
	it("ignores package.json without smolt field, falls back to index.ts", async () => {
		const subdir = path.join(extensionsDir, "my-package");
		fs.mkdirSync(subdir);
		fs.writeFileSync(path.join(subdir, "index.ts"), extensionCode);
		fs.writeFileSync(
	...
	it("handles mixed direct files and subdirectories", async () => {
		...
		fs.writeFileSync(path.join(subdir2, "package.json"), JSON.stringify({ smolt: { extensions: ["./entry.ts"] } }));
	...
	it("registers message and entry renderers", async () => {
		const extCode = `
			export default function(smolt) {
				smolt.registerMarkdownTransformer((markdown) => {
					return markdown;
				});
				smolt.registerMessageRenderer("my-custom-type", (message, options, theme) => {
					return null; // Use default rendering
				});
				smolt.registerEntryRenderer("my-entry-type", (entry, options, theme) => {
					return null;
				});
			}
		`;
	...
	it("reports error when extension has no default export", async () => {
		const extCode = `
			export function notDefault(smolt) {
				smolt.registerCommand("test", { handler: async () => {} });
			}
		`;
	...
	it("loads extension with event handlers", async () => {
		const extCode = `
			export default function(smolt) {
				...
				smolt.on("tool_call", async (event) => undefined);
				smolt.on("agent_end", async () => {});
			}
		`;
