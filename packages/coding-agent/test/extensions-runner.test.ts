...
describe("ExtensionRunner", () => {
	...
	beforeEach(async () => {
		tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "smolt-runner-test-"));
	...
	smolt.on("project_trust", () => ({ trusted: "undecided", remember: true }));
}`,
			);
			fs.writeFileSync(
				decidedPath,
				`export default function(smolt) {
	smolt.on("project_trust", () => ({ trusted: "no", remember: true }));
}`,
			);
	...
	describe("shortcut conflicts", () => {
		...
		it("warns but allows when extension uses non-reserved built-in shortcut", async () => {
			...
			const extCode = `
				export default function(smolt) {
					smolt.registerShortcut("${pasteImageKey}", {
		...
		it("warns when two extensions register same shortcut", async () => {
			...
			const extCode1 = `
				export default function(smolt) {
					smolt.registerShortcut("ctrl+shift+x", {
	...
	describe("command collection", () => {
		...
		it("suffixes duplicate extension commands in insertion order", async () => {
			const cmdCode = (description: string) => `
				export default function(smolt) {
					smolt.registerCommand("shared-cmd", {
	...
	describe("message and entry renderers", () => {
		it("gets Markdown transformers in extension load order", async () => {
			const extCode = `
				export default function(smolt) {
					smolt.registerMarkdownTransformer((markdown) => markdown);
				}
			`;
		...
		it("gets message renderer by type", async () => {
			const extCode = `
				export default function(smolt) {
					smolt.registerMessageRenderer("my-type", (message, options, theme) => null);
				}
			`;
		...
		it("gets entry renderer by type", async () => {
			const extCode = `
				export default function(smolt) {
					smolt.registerEntryRenderer("my-entry", (entry, options, theme) => null);
				}
			`;
	...
	describe("flags", () => {
		...
		it("keeps first flag when two extensions register the same name", async () => {
			const first = `
				export default function(smolt) {
					smolt.registerFlag("shared-flag", {
						description: "first",
						type: "boolean",
						default: true,
					});
	...
	describe("hasHandlers", () => {
		it("returns true when handlers exist for event type", async () => {
			const extCode = `
				export default function(smolt) {
					smolt.on("tool_call", async () => undefined);
				}
			`;
	...
	describe("before_provider_headers", () => {
		it("lets a handler mutate headers in place and preserves existing headers", async () => {
			const extCode = `
				export default function(smolt) {
					smolt.on("before_provider_headers", (event) => {
		...
		it("isolates a throwing handler and still applies the others", async () => {
			const throwing = `
				export default function(smolt) {
					smolt.on("before_provider_headers", () => {
