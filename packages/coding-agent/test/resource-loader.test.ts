...
Skill content here.`,
		...
		it("should ignore extra markdown files in auto-discovered skill dirs", async () => {
			const skillDir = join(agentDir, "skills", "smolt-skills", "browser-tools");
...
Prompt content.`,
		...
		it("should prefer project resources over user on name collisions", async () => {
			...
			const projectPromptsDir = join(cwd, ".smolt", "prompts");
			...
			const projectSkillDir = join(cwd, ".smolt", "skills", "collision-skill");
...
Project skill`,
			...
			const projectThemePath = join(cwd, ".smolt", "themes", "collision.json");
			mkdirSync(join(agentDir, "themes"), { recursive: true });
			mkdirSync(join(cwd, ".smolt", "themes"), { recursive: true });
	...
	smolt.registerCommand("shared", {
		...
		handler: async () => {},
			...
			mkdirSync(agentDir, { recursive: true });
			mkdirSync(join(cwd, ".smolt"), { recursive: true });
			symlinkSync(sharedExtDir, join(agentDir, "extensions"), "dir");
			symlinkSync(sharedExtDir, join(cwd, ".smolt", "extensions"), "dir");
			...
			expect(extensionsResult.extensions[0].path).toBe(join(cwd, ".smolt", "extensions", "shared.ts"));
		});

		it("should load user extensions before trust and reuse them after trust resolves", async () => {
			const userExtDir = join(agentDir, "extensions");
			const projectExtDir = join(cwd, ".smolt", "extensions");
...
export default function(smolt) {
	...
	smolt.registerCommand("user-trust", {
		...
		handler: async () => {},
			...
			writeFileSync(
				...
				`export default function(smolt) {
	smolt.registerCommand("project-trusted", {
		...
		handler: async () => {},
			...
			const extensionsResult = loader.getExtensions();
			expect(extensionsResult.extensions.map((extension) => extension.path)).toEqual([
				join(cwd, ".smolt", "extensions", "project.ts"),
				join(userExtDir, "user.ts"),
			]);
	...
	smolt.registerCommand("project-only", {
	...
	smolt.registerCommand("user-only", {
...
Content`,
		...
		it("should discover SYSTEM.md from cwd/.smolt", async () => {
			const smoltDir = join(cwd, ".smolt");
			mkdirSync(smoltDir, { recursive: true });
			writeFileSync(join(smoltDir, "SYSTEM.md"), "You are a helpful assistant.");
		...
		it("should skip project resources that require trust when project is not trusted", async () => {
			const smoltDir = join(cwd, ".smolt");
			const extensionsDir = join(smoltDir, "extensions");
			const skillDir = join(smoltDir, "skills", "project-skill");
			const promptsDir = join(smoltDir, "prompts");
			const themesDir = join(smoltDir, "themes");
			mkdirSync(extensionsDir, { recursive: true });
			mkdirSync(skillDir, { recursive: true });
			mkdirSync(promptsDir, { recursive: true });
			mkdirSync(themesDir, { recursive: true });
			writeFileSync(join(smoltDir, "SYSTEM.md"), "Project system prompt.");
...
Project skill content`,
		...
		it("should discover APPEND_SYSTEM.md", async () => {
			const smoltDir = join(cwd, ".smolt");
			mkdirSync(smoltDir, { recursive: true });
			writeFileSync(join(smoltDir, "APPEND_SYSTEM.md"), "Additional instructions.");
	...
	describe("system prompt sources", () => {
		it("exposes discovered project SYSTEM.md as the system prompt source", async () => {
			const smoltDir = join(cwd, ".smolt");
			const systemPromptPath = join(smoltDir, "SYSTEM.md");
			mkdirSync(smoltDir, { recursive: true });
		...
		it("exposes discovered APPEND_SYSTEM.md as an append system prompt source", async () => {
			const smoltDir = join(cwd, ".smolt");
			const appendSystemPromptPath = join(smoltDir, "APPEND_SYSTEM.md");
			mkdirSync(smoltDir, { recursive: true });
