...
- Variables such as `PI_OFFLINE` configure the Smolt process.
...
- `PI_CODING_AGENT=true` is Smolt-specific and lets child processes detect that they run inside Smolt.
...
The values are resolved when each command starts. Switching models or changing the reasoning level therefore affects the next shell command without restarting Smolt. `PI_PROVIDER` and `PI_MODEL` identify the selected Smolt model, not a different upstream model that a router may choose internally.
...
| Variable | Description |
|----------|-------------|
| `PI_CODING_AGENT_DIR` | Override the config directory; default is `~/.smolt/agent` |
