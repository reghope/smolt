...
describe("package commands", () => {
	...
	let originalSmoltPackageDir: string | undefined;
	...
	function prepareManagedInstall(
		...
		writeFileSync(
			...
			`${JSON.stringify({ kind: "smolt-managed-install", schemaVersion: 1, layout: "releases-v1" })}\n`,
		);
...
const smoltPath = path.join(binDir, process.platform === "win32" ? "smolt.cmd" : "smolt");
fs.writeFileSync(
	smoltPath,
...
if (process.platform !== "win32") fs.chmodSync(smoltPath, 0o755);
`,
		);
		...
		vi.stubEnv("SMOLT_INSTALLER_API_BASE", "https://example.test/api/installer/releases");
		vi.stubEnv("SMOLT_MANAGED_INSTALL_ROOT", managedRoot);
		process.env.SMOLT_PACKAGE_DIR = selfPackageDir;
	...
	beforeEach(() => {
		allowNetwork();
		tempDir = join(tmpdir(), `smolt-package-commands-${Date.now()}-${Math.random().toString(36).slice(2)}`);
		...
		originalSmoltPackageDir = process.env.SMOLT_PACKAGE_DIR;
	...
	it("skips untrusted project package settings", async () => {
		...
		writeFileSync(join(projectDir, ".smolt", "settings.json"), JSON.stringify({ packages: ["npm:@project/pkg"] }));
		...
		try {
			await expect(
				main(["list"], {
					extensionFactories: [
						(smolt) => {
							smolt.on("project_trust", () => ({ trusted: "yes" }));
						},
					],
				}),
	...
	it("does not prompt or ask extensions for project trust during update", async () => {
		...
		writeFileSync(
			join(projectDir, ".smolt", "settings.json"),
	...
	it("blocks local package changes when project is untrusted", async () => {
		mkdirSync(join(projectDir, ".smolt"), { recursive: true });
		writeFileSync(join(projectDir, ".smolt", "settings.json"), "{}");
	...
	it("allows local package install to initialize fresh project settings", async () => {
		...
		const settingsPath = join(projectDir, ".smolt", "settings.json");
		const settings = JSON.parse(readFileSync(settingsPath, "utf-8")) as { packages?: string[] };
		expect(settings.packages?.length).toBe(1);
		const stored = settings.packages?.[0] ?? "";
		expect(realpathSync(join(projectDir, ".smolt", stored))).toBe(realpathSync(packageDir));
		expect(process.exitCode).toBeUndefined();
	});
	...
	it("shows install subcommand help", async () => {
		...
		try {
			...
			expect(stdout).toContain("smolt install <source> [-l]");
	...
	it("cycles project package overrides in config local mode", async () => {
		...
		storage.withLock("global", () => JSON.stringify({ packages: ["npm:smolt-tools"] }));
		const settingsManager = SettingsManager.fromStorage(storage, { projectTrusted: true });
		const resolvedPaths = extensionPaths(join(tempDir, "pkg"), "npm:smolt-tools", "user", ["bar.ts"]);
		...
		expect(settingsManager.getProjectSettings().packages).toEqual([
			{ source: "npm:smolt-tools", autoload: false, extensions: ["-extensions/bar.ts"] },
		]);
	...
	it("shows a friendly error for unknown install options", async () => {
		...
		try {
			...
			expect(stderr).toContain('Use "smolt --help" or "smolt install <source> [-l] [--approve|--no-approve]".');
			expect(process.exitCode).toBe(1);
		} finally {
			errorSpy.mockRestore();
		}
	...
	it("shows a friendly error for missing install source", async () => {
		...
		try {
			...
			expect(stderr).toContain("Usage: smolt install <source> [-l]");
	...
	it("allows explicit self-update checks when automatic version checks are disabled", async () => {
		const previousSkipVersionCheck = process.env.SMOLT_SKIP_VERSION_CHECK;
		process.env.SMOLT_SKIP_VERSION_CHECK = "1";
		...
		try {
			...
			expect(logSpy.mock.calls.map(([message]) => String(message)).join("\n")).toContain(
				`smolt is already up to date (v${VERSION})`,
			);
			expect(errorSpy).not.toHaveBeenCalled();
			expect(process.exitCode).toBeUndefined();
		} finally {
			if (previousSkipVersionCheck === undefined) {
				delete process.env.SMOLT_SKIP_VERSION_CHECK;
			} else {
				process.env.SMOLT_SKIP_VERSION_CHECK = previousSkipVersionCheck;
			}
		}
	});
	...
	it("retries a transient self-update version check", async () => {
		...
		try {
			...
			if (previousSkipVersionCheck === undefined) delete process.env.SMOLT_SKIP_VERSION_CHECK;
			else process.env.SMOLT_SKIP_VERSION_CHECK = previousSkipVersionCheck;
			logSpy.mockRestore();
			errorSpy.mockRestore();
		}
	});

	it("updates installer-managed Smolt through a staged immutable release", async () => {
	...
	it("rejects a concurrent managed update", async () => {
		...
		expect(logSpy.mock.calls.map(([message]) => String(message)).join("\n")).not.toContain("Updated smolt from");
		expect(errorSpy.mock.calls.map(([message]) => String(message)).join("\n")).toContain(
			"Another managed Smolt update is already running.",
		);
		expect(process.exitCode).toBe(1);
	});
	...
	it("rejects forced managed reinstalls", async () => {
		...
		expect(existsSync(npmRecordPath)).toBe(false);
		expect(errorSpy.mock.calls.map(([message]) => String(message)).join("\n")).toContain(
			"Managed smolt installations do not support --force",
		);
		expect(process.exitCode).toBe(1);
	});
	...
	it("keeps npm self-updates non-managed when the managed environment is inherited", async () => {
		...
		const selfPackageDir = join(globalPrefix, "lib", "node_modules", "@earendil-works", "smolt-coding-agent");
		...
		vi.stubEnv("SMOLT_MANAGED_INSTALL_ROOT", inheritedManagedRoot);
...
`,
		...
		try {
			...
			expect(stdout).toContain(`Updated smolt from ${VERSION} to ${VERSION}`);
		} finally {
			logSpy.mockRestore();
			errorSpy.mockRestore();
		}
	...
	it("uses the current package name when the update check omits packageName", async () => {
		const globalPrefix = join(tempDir, "global-prefix");
		const selfPackageDir = join(globalPrefix, "lib", "node_modules", "@mariozechner", "smolt-coding-agent");
		const fakeNpmPath = join(tempDir, "fake-npm.cjs");
		const recordPath = join(tempDir, "self-update.json");
		mkdirSync(selfPackageDir, { recursive: true });
		writeFileSync(
...
`,
		...
		try {
			...
			expect(recordedArgs).not.toContain(PACKAGE_NAME);
			expect(stdout).toContain(`Updated smolt from ${VERSION} to ${targetVersion}`);
		} finally {
			logSpy.mockRestore();
			errorSpy.mockRestore();
		}
...
`,
		...
		const activePackageName = PACKAGE_NAME === "@new-scope/smolt" ? "@newer-scope/smolt" : "@new-scope/smolt";
		vi.stubGlobal(
			"fetch",
	...
	it("prints a pnpm metadata hint when self-update fails", async () => {
		...
		const selfPackageDir = join(globalRoot, "node_modules", "@earendil-works", "smolt-coding-agent");
		...
		try {
			...
			const stdout = logSpy.mock.calls.map(([message]) => String(message)).join("\n");
			const stderr = errorSpy.mock.calls.map(([message]) => String(message)).join("\n");
			expect(stdout).not.toContain("Updated smolt");
			expect(stderr).toContain("exited with code 23");
			expect(stderr).toContain("If pnpm reports missing package versions");
			expect(stderr).toContain("Run `pnpm store prune` and retry `smolt update --self`.");
		} finally {
			logSpy.mockRestore();
			errorSpy.mockRestore();
		}
...
`,
	...
	it("suggests the configured source when update input omits the npm prefix", async () => {
		const settingsPath = join(agentDir, "settings.json");
		writeFileSync(settingsPath, JSON.stringify({ packages: ["npm:smolt-formatter"] }, null, 2));

		const errorSpy = vi.spyOn(console, "error").mockImplementation(() => {});
		const logSpy = vi.spyOn(console, "log").mockImplementation(() => {});

		try {
			await expect(main(["update", "smolt-formatter"])).resolves.toBeUndefined();

			const stderr = errorSpy.mock.calls.map(([message]) => String(message)).join("\n");
			const stdout = logSpy.mock.calls.map(([message]) => String(message)).join("\n");
			expect(stderr).toContain("Did you mean npm:smolt-formatter?");
			expect(stdout).not.toContain("Updated smolt-formatter");
			expect(process.exitCode).toBe(1);

			const settings = JSON.parse(readFileSync(settingsPath, "utf-8")) as { packages?: string[] };
			expect(settings.packages).toContain("npm:smolt-formatter");
		} finally {
			errorSpy.mockRestore();
			logSpy.mockRestore();
		}
