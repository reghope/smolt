...
import { fauxAssistantMessage, registerFauxProvider } from "@smolt/ai/compat";
...
describe("regression #2860: replaced session callbacks", () => {
	...
	async function createRuntimeForTest(extensionFactory: ExtensionFactory, responses: string[]) {
		const tempDir = join(tmpdir(), `smolt-2860-${Date.now()}-${Math.random().toString(36).slice(2)}`);
	...
	it("rebinds before withSession, targets the replacement session, and invalidates stale smolt/ctx", async () => {
		const events: string[] = [];
		let oldCtx: ExtensionCommandContext | undefined;
		let oldSmolt: ExtensionAPI | undefined;
		let oldSessionFile: string | undefined;
		let staleCtxThrows = false;
		let staleSmoltThrows = false;
		...
		const { runtime } = await createRuntimeForTest(
			(smolt) => {
				...
				smolt.on("session_shutdown", () => {
					events.push(`shutdown:${currentInstance}`);
				});
				smolt.registerCommand("repro", {
					description: "repro",
					handler: async (_args, ctx) => {
						...
						await ctx.newSession({
							...
							withSession: async (replacedCtx) => {
								...
								try {
									oldSmolt?.sendUserMessage("stale message");
								} catch {
									staleSmoltThrows = true;
								}
		...
		expect(staleSmoltThrows).toBe(true);
	...
	it("supports withSession for switchSession", async () => {
		...
		const { runtime } = await createRuntimeForTest(
			(smolt) => {
				smolt.registerCommand("switch-it", {
