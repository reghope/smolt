...
import { type PiMessagesOptions, stream, streamSimple } from "../src/api/smolt-messages.ts";
...
function createModel(baseUrl: string): Model<"smolt-messages"> {
	return {
		id: "auto",
		name: "Radius Auto",
		api: "smolt-messages",
...
describe("smolt-messages", () => {
	...
	it("appends debug=1 and reports response headers via onResponse", async () => {
		const { baseUrl, requests } = await startServer({
			headers: { "x-smolt-gateway-upstream-provider": "anthropic" },
		...
		expect(observedHeaders?.["x-smolt-gateway-upstream-provider"]).toBe("anthropic");
	});
	...
	it("surfaces backend error responses with diagnostics", async () => {
		...
		expect(message.diagnostics?.[0]?.type).toBe("smolt_messages_response_failure");
...
describe("smolt-messages api registration", () => {
	it("is registered as a builtin api provider", async () => {
		const { getApiProvider } = await import("../src/compat.ts");
		expect(getApiProvider("smolt-messages")).toBeDefined();
	});

	it("is a known api usable on models", () => {
		const api: Api = "smolt-messages";
		expect(api).toBe("smolt-messages");
	});
});
