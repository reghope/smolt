...
Testers are kept lean by `lean.ts`, an inline extension every child session runs with: each tool result is capped at 10,000 tokens as it enters history, tool results older than the four most recent (screenshots included) collapse to a one-line stub before each model call — in batches of four, so the cached prompt prefix survives between batches — and the testlog calls behind shed results are trimmed with them, so a ticket's prose does not ride on every later turn. The project's AGENTS.md stays out of the child's prompt, and the tools extension's reading habits ride on it instead. A tester's spend, in the roster, the footer and every `wait`'s usage, counts every token billed — the re-read context that comes back as cache reads included.
...
```
~/.smolt/projects/<project>/battletest/<run-slug>/
