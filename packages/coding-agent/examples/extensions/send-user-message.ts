 ...
 *
 * Demonstrates smolt.sendUserMessage() for sending user messages from extensions.
 * Unlike smolt.sendMessage() which sends custom messages, sendUserMessage() sends
...
export default function (smolt: ExtensionAPI) {
	...
	smolt.registerCommand("ask", {
		...
		handler: async (args, ctx) => {
			...
			smolt.sendUserMessage(args);
		},
	});

	// Command that steers the agent mid-conversation
	smolt.registerCommand("steer", {
		...
		handler: async (args, ctx) => {
			...
			if (ctx.isIdle()) {
				...
				smolt.sendUserMessage(args, { deliverAs: "steer" });
			}
		},
	});

	// Command that queues a follow-up message
	smolt.registerCommand("followup", {
		...
		handler: async (args, ctx) => {
			...
			if (ctx.isIdle()) {
				...
				smolt.sendUserMessage(args, { deliverAs: "followUp" });
