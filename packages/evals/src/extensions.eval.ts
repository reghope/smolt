...
import { createSmoltCodingAgentHarness, type PiCodingAgentInput } from "./smolt-harness.ts";
...
type ExtensionAuthoringOutput = {
	...
	systemPromptHasSmoltDocs: boolean;
	...
	return createSmoltCodingAgentHarness({
		...
		output: ({ response, session }) => {
			...
			return {
				...
				systemPromptHasSmoltDocs: session.systemPrompt.includes("\nSmolt documentation (read only"),
...
function excludeGuidelinesAndDocumentation(defaultPrompt: string): string {
	...
	if (guidelinesStart === -1) throw new Error("Default Smolt system prompt has no Guidelines section.");
...
function prepareDefaultPromptOverride(defaultPrompt: string): string {
	...
	if (cwdStart === -1) throw new Error("Default Smolt system prompt has no working-directory section.");
...
const extensionHarnessTable = evalHarnessTable("Smolt extension authoring system prompt", {
...
describe.for(extensionHarnessTable)("$name", ({ harness }) => {
	describeEval(
		"Smolt extension authoring system prompt",
		...
		(it) => {
			it("creates, reloads, and uses a hello extension", async ({ run, task }) => {
				const result = await run([
					{
						type: "prompt",
						content:
							"Create a Smolt extension with a hello tool that takes a name and returns a greeting. For example, passing Bob should return `Hello, Bob!`.",
					},
					{ type: "reload" },
					{
						type: "prompt",
				...
				if (result.output.extensionSource !== null) {
					...
					if (typeof runId !== "string") throw new Error("Smolt eval run did not record a run ID.");
				...
				expect(result.output.systemPromptHasSmoltDocs).toBe(expectsFullPrompt);
			});
		},
	);
});
