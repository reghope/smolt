/**
 * smolt-messages API implementation.
 *
 * Streams smolt's own message protocol directly to a backend: the request is a
 ...
 * models.json custom provider with `"api": "smolt-messages"`.
 */
...
/** Serialized assistant-message event as sent by a smolt-messages backend. */
...
function parseSmoltMessagesErrorBody(body: string): PiMessagesErrorBody | undefined {
	try {
...
function formatSmoltMessagesResponseError(
	response: Response,
	body: string,
...
function createSmoltMessagesResponseError(
	model: Model<"smolt-messages">,
	url: URL,
	response: Response,
	body: string,
): PiMessagesResponseError {
	const errorBody = parseSmoltMessagesErrorBody(body);
	const code = typeof errorBody?.error?.code === "string" ? errorBody.error.code : undefined;
	return new PiMessagesResponseError(formatSmoltMessagesResponseError(response, body, errorBody), code, {
		version: 1,
...
function appendRewriteDiagnostic(message: AssistantMessage, rewrite: PiMessagesRewriteImpact | undefined): void {
	...
	appendAssistantMessageDiagnostic(message, {
		type: "smolt_messages_rewrite",
		timestamp: Date.now(),
		details: { ...rewrite },
	});
}

function createEventConverter(model: Model<"smolt-messages">) {
...
async function* readSmoltMessagesEvents(stream: ReadableStream<Uint8Array>): AsyncGenerator<PiMessagesEvent> {
	...
	try {
		while (true) {
			...
			while (split !== -1) {
				const event = parseSmoltMessagesEvent(buffer.slice(0, split));
				if (event) {
					yield event;
				}
		...
		if (buffer.trim()) {
			const event = parseSmoltMessagesEvent(buffer);
			if (event) {
				yield event;
			}
		}
...
function parseSmoltMessagesEvent(raw: string): PiMessagesEvent | undefined {
	const data = raw
		.split("\n")
...
function createErrorEvent(model: Model<"smolt-messages">, error: unknown, aborted: boolean): AssistantMessageEvent {
	...
	if (!aborted && error instanceof PiMessagesResponseError) {
		appendAssistantMessageDiagnostic(
			assistantMessage,
			createAssistantMessageDiagnostic("smolt_messages_response_failure", error, error.diagnosticDetails),
		);
	}
	...
	void (async () => {
		try {
			...
			if (!response.ok) {
				...
				throw createSmoltMessagesResponseError(model, url, response, body);
			}
			if (!response.body) {
				throw new Error(`${model.provider} response has no body`);
			}

			for await (const smoltEvent of readSmoltMessagesEvents(response.body)) {
				const event = convertEvent(smoltEvent);
...
export const streamSimple: StreamFunction<"smolt-messages", SimpleStreamOptions> = (
	model: Model<"smolt-messages">,
	context: Context,
