...
describe("AgentSession dynamic provider registration", () => {
	...
	beforeEach(() => {
		tempDir = join(tmpdir(), `smolt-dynamic-provider-test-${Date.now()}-${Math.random().toString(36).slice(2)}`);
	...
	it("applies top-level registerProvider overrides to the active model", async () => {
		const session = await createSession([
			(smolt) => {
				smolt.registerProvider("anthropic", { baseUrl: "http://localhost:8080/top-level" });
			},
		]);
		...
		const session = await createSession([
			(smolt) => {
				smolt.on("session_start", () => {
					smolt.registerProvider("anthropic", { baseUrl: "http://localhost:8080/session-start" });
				});
			},
		]);
	...
	it("registers native smolt-ai providers during extension loading", async () => {
		const session = await createSession([
			(smolt) => {
				smolt.registerProvider(nativeAnthropicProvider("http://localhost:8080/native-top-level"));
			},
		]);
	...
	it("applies command-time registerProvider overrides without reload", async () => {
		const session = await createSession([
			(smolt) => {
				smolt.registerCommand("use-proxy", {
					description: "Use proxy",
					handler: async () => {
						smolt.registerProvider("anthropic", { baseUrl: "http://localhost:8080/command" });
					},
				});
			},
		]);
	...
	it("registers native smolt-ai providers at command time", async () => {
		const session = await createSession([
			(smolt) => {
				smolt.registerCommand("use-native", {
					description: "Use native provider",
					handler: async () => {
						smolt.registerProvider(nativeAnthropicProvider("http://localhost:8080/native-command"));
					},
				});
			},
		]);
