...
import type { AgentMessage, AgentTool } from "@smolt/agent-core";
import { Agent } from "@smolt/agent-core";
import type { FauxModelDefinition, FauxProviderRegistration, FauxResponseStep, Model } from "@smolt/ai/compat";
import { registerFauxProvider, streamSimple } from "@smolt/ai/compat";
...
function createTempDir(): string {
	const tempDir = join(tmpdir(), `smolt-suite-${Date.now()}-${Math.random().toString(36).slice(2)}`);
