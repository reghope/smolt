...
import { getBuiltinModels, getBuiltinProviders } from "@smolt/ai/providers/all";
...
describe("default model selection", () => {
	...
	describe("persisted default model scoping", () => {
		...
		async function createSession(options: { scoped: boolean; persistedScope?: string[] }) {
			const tempDir = join(tmpdir(), `smolt-default-scope-${Date.now()}-${Math.random().toString(36).slice(2)}`);
