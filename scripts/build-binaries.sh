#!/usr/bin/env bash
#
# Build smolt binaries for all platforms locally.
...
#     smolt-darwin-arm64.tar.gz
#     smolt-darwin-x64.tar.gz
#     smolt-linux-x64.tar.gz
#     smolt-linux-arm64.tar.gz
#     smolt-windows-x64.zip
#     smolt-windows-arm64.zip
...
for platform in "${PLATFORMS[@]}"; do
    ...
    # standalone binary before smolt starts (see #7684).
    if [[ "$platform" == windows-* ]]; then
        bun build --compile --no-compile-autoload-bunfig --target="$bun_target" ./dist/bun/cli.js ./src/utils/image-resize-worker.ts --outfile "$OUTPUT_DIR/$platform/smolt.exe"
    else
        bun build --compile --no-compile-autoload-bunfig --target="$bun_target" ./dist/bun/cli.js ./src/utils/image-resize-worker.ts --outfile "$OUTPUT_DIR/$platform/smolt"
    fi
done
...
for platform in "${PLATFORMS[@]}"; do
    if [[ "$platform" == windows-* ]]; then
        # Windows (zip)
        echo "Creating smolt-$platform.zip..."
        (cd "$platform" && zip -r ../smolt-$platform.zip .)
    else
        # Unix platforms (tar.gz) - use wrapper directory for mise compatibility
        echo "Creating smolt-$platform.tar.gz..."
        mv "$platform" smolt && tar -czf smolt-$platform.tar.gz smolt && mv smolt "$platform"
    fi
done
...
for platform in "${PLATFORMS[@]}"; do
    rm -rf "$platform"
    if [[ "$platform" == windows-* ]]; then
        mkdir -p "$platform" && (cd "$platform" && unzip -q ../smolt-$platform.zip)
    else
        tar -xzf smolt-$platform.tar.gz && mv smolt "$platform"
    fi
done
...
for platform in "${PLATFORMS[@]}"; do
    if [[ "$platform" == windows-* ]]; then
        echo "  $OUTPUT_DIR/$platform/smolt.exe"
    else
        echo "  $OUTPUT_DIR/$platform/smolt"
    fi
done
