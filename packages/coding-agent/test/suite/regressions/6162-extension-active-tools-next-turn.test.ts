...
describe("extension active tools next-turn refresh", () => {
	it("applies smolt.setActiveTools before the next provider request in the same run", async () => {
		const extensionFactories: ExtensionFactory[] = [
			(smolt) => {
				smolt.registerTool({
					...
					execute: async () => {
						smolt.setActiveTools(["after_switch"]);
						return {
		...
		const extensionFactories: ExtensionFactory[] = [
			(smolt) => {
				smolt.registerTool({
					...
					parameters: Type.Object({}),
					execute: async () => {
						smolt.setActiveTools([...smolt.getActiveTools(), "after_load"]);
						return {
