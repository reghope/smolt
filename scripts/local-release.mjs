...
const packages = [
	{ directory: "packages/telemetry", name: "@smolt/telemetry" },
	{ directory: "packages/ai", name: "@smolt/ai" },
	{ directory: "packages/tui", name: "@smolt/tui" },
	{ directory: "packages/agent", name: "@smolt/agent-core" },
	{ directory: "packages/protocol", name: "@smolt/protocol" },
...
function prepareOutputDirectory(options, repoRoot) {
	if (!options.outDir) {
		return mkdtempSync(join(tmpdir(), "smolt-local-release-"));
	}
...
function buildBunBinaryRelease(targetDirectory, archiveDirectory) {
	...
	const archiveName = platform.startsWith("windows-") ? `smolt-${platform}.zip` : `smolt-${platform}.tar.gz`;
	cpSync(join(binaryBuildDirectory, archiveName), join(archiveDirectory, archiveName));
	return platform;
}

function createSmoltShim(installDirectory) {
	const binDirectory = join(installDirectory, "node_modules", ".bin");
	if (process.platform === "win32") {
		if (existsSync(join(binDirectory, "smolt.cmd"))) {
			writeFileSync(join(installDirectory, "smolt.cmd"), '@ECHO off\r\n"%~dp0node_modules\\.bin\\smolt.cmd" %*\r\n');
			writeFileSync(join(installDirectory, "smolt.ps1"), '& "$PSScriptRoot/node_modules/.bin/smolt.ps1" @args\n');
			return;
		}
		writeFileSync(join(installDirectory, "smolt.cmd"), '@ECHO off\r\n"%~dp0node_modules\\.bin\\smolt.exe" %*\r\n');
		writeFileSync(join(installDirectory, "smolt.ps1"), '& "$PSScriptRoot/node_modules/.bin/smolt.exe" @args\n');
		return;
	}
	symlinkSync(join("node_modules", ".bin", "smolt"), join(installDirectory, "smolt"));
}
...
if (rootPackageJson.name !== "smolt-monorepo") {
...
if (!options.skipInstall) {
	...
	console.log(`  ${join(outDir, `smolt-${binaryPlatform}.${String(binaryPlatform).startsWith("windows-") ? "zip" : "tar.gz"}`)}`);
	console.log("\nRun the local Bun binary release from outside the repository:");
	console.log(`  ${join(binaryDirectory, String(binaryPlatform).startsWith("windows-") ? "smolt.exe" : "smolt")} --help`);

	console.log("\nIsolated npm install:");
	console.log(`  ${nodeInstallDirectory}`);
	console.log("\nRun the locally packed npm CLI from outside the repository:");
	console.log(`  ${join(nodeInstallDirectory, process.platform === "win32" ? "smolt.cmd" : "smolt")} --help`);
	...
	if (!options.skipBunInstall) {
		...
		console.log(`  ${join(bunInstallDirectory, process.platform === "win32" ? "smolt.cmd" : "smolt")} --help`);
	}
}
