 ...
 *
 * Usage:
 *   smolt -e ./ssh.ts --ssh user@host
 *   smolt -e ./ssh.ts --ssh user@host:/remote/path
 *
 * Requirements:
...
export default function (smolt: ExtensionAPI) {
	smolt.registerFlag("ssh", { description: "SSH remote: user@host or user@host:/path", type: "string" });
	...
	smolt.on("session_start", async (_event, ctx) => {
		...
		const arg = smolt.getFlag("ssh") as string | undefined;
		if (arg) {
			if (arg.includes(":")) {
