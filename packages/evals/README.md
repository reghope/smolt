...
Smolt evals are behavioral, model-backed checks for Smolt workflows. They adapt a real `AgentSession` to `vitest-evals`, run
it in isolated temporary project and agent directories, and attach native Smolt session artifacts.
...
Authentication comes from Smolt's normal `ModelRuntime`, including Smolt subscription credentials and provider API-key
...
native Smolt session JSONL attachments under `sessions/`. These files may contain prompts, responses, source code, and tool
output.

## Writing evals

Follow [`vitest-evals`](https://github.com/getsentry/vitest-evals) for general suite, judge, assertion, and normalized
trace guidance. Smolt-specific evals use `createSmoltCodingAgentHarness(...)` from `src/smolt-harness.ts`, with one harness bound
...
const harness = createSmoltCodingAgentHarness({ noTools: "all" });

describeEval("Smolt smoke", { harness }, (it) => {
...
### Configuring the Smolt harness

`createSmoltCodingAgentHarness(...)` accepts:
...
- `noTools`: Smolt's tool-disable configuration.
...
```ts
const harness = createSmoltCodingAgentHarness({
...
prompt creates or changes Smolt resources:

```ts
const result = await run([
	{ type: "prompt", content: "Create a Smolt extension." },
	{ type: "reload" },
...
Use `output` to expose scenario-specific, JSON-safe behavior without adding that behavior to the generic Smolt adapter:
...
Harnesses may differ by prompt, tools, skills, model, or any other Smolt configuration:
...
The Smolt harness snapshots native session JSONL before deleting its temporary workspace. An eval-only `afterEach` hook
