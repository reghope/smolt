...
import type { OAuthCredentials } from "@smolt/ai";
import { getModel, streamSimple } from "@smolt/ai/compat";
import { builtinProviders } from "@smolt/ai/providers/all";
...
// ============================================================================
// OAuth API key resolution from ~/.smolt/agent/auth.json
// ============================================================================

const AUTH_PATH = join(homedir(), ".smolt", "agent", "auth.json");
...
/**
 * Resolve API key for a provider from ~/.smolt/agent/auth.json
 *
...
/**
 * Check if a provider has credentials in ~/.smolt/agent/auth.json
 */
...
export async function createTestSession(options: TestSessionOptions = {}): Promise<TestSessionContext> {
	const tempDir = join(tmpdir(), `smolt-test-${Date.now()}-${Math.random().toString(36).slice(2)}`);
