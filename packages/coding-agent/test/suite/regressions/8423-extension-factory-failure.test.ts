...
describe("issue #8423 extension factory failure", () => {
	it("discards runtime changes and disables the failed API", async () => {
		...
		await loadExtensionFromFactory(
			(smolt) => smolt.registerProvider("working-provider", providerConfig),
			process.cwd(),
			eventBus,
			runtime,
			"<working>",
		...
		await expect(
			loadExtensionFromFactory(
				(smolt) => {
					capturedApi = smolt;
					smolt.events.on("factory-failure", () => {
						eventCalls++;
					});
					smolt.registerFlag("failed-flag", { type: "boolean", default: true });
					flagDuringLoad = smolt.getFlag("failed-flag");
					smolt.unregisterProvider("working-provider");
					smolt.registerProvider("failed-provider", providerConfig);
