...
export default function (smolt: ExtensionAPI) {
	smolt.on("session_shutdown", async (_event, ctx) => {
		...
		const { stdout: status, code } = await smolt.exec("git", ["status", "--porcelain"]);
		...
		const commitMessage = `[smolt] ${firstLine.slice(0, 50)}${firstLine.length > 50 ? "..." : ""}`;

		// Stage and commit
		await smolt.exec("git", ["add", "-A"]);
		const { code: commitCode } = await smolt.exec("git", ["commit", "-m", commitMessage]);
