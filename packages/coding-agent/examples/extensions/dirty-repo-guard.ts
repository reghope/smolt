...
async function checkDirtyRepo(
	...
	const { stdout, code } = await smolt.exec("git", ["status", "--porcelain"]);
...
export default function (smolt: ExtensionAPI) {
	smolt.on("session_before_switch", async (event, ctx) => {
		...
		return checkDirtyRepo(smolt, ctx, action);
	});

	smolt.on("session_before_fork", async (_event, ctx) => {
		return checkDirtyRepo(smolt, ctx, "fork");
	});
}
