...
export default function (smolt: ExtensionAPI) {
	// Register a /quit command that cleanly exits smolt
	smolt.registerCommand("quit", {
		description: "Exit smolt cleanly",
	...
	smolt.registerTool({
		...
		description: "Complete a task and exit smolt",
	...
	smolt.registerTool({
		...
		description: "Deploy the application and exit smolt",
		parameters: Type.Object({
		...
		async execute(_toolCallId, params, _signal, onUpdate, ctx) {
			...
			// const result = await smolt.exec("npm", ["run", "deploy", params.environment], { signal });
