 ...
 * - ~/.smolt/agent/presets.json (global)
 * - <cwd>/.smolt/presets.json (project-local)
 *
 ...
 *
 * Usage:
 * - `smolt --preset plan` - start with plan preset
...
export default function presetExtension(smolt: ExtensionAPI) {
	...
	async function applyPreset(name: string, preset: Preset, ctx: ExtensionContext): Promise<boolean> {
		...
		if (activePresetName === undefined) {
			originalState = {
				model: ctx.model,
				thinkingLevel: smolt.getThinkingLevel(),
				tools: smolt.getActiveTools(),
			};
		}
		...
		if (preset.thinkingLevel) {
			smolt.setThinkingLevel(preset.thinkingLevel);
		}

		// Apply tools if specified
		if (preset.tools && preset.tools.length > 0) {
			const allToolNames = smolt.getAllTools().map((t) => t.name);
			...
			if (validTools.length > 0) {
				smolt.setActiveTools(validTools);
			}
		}
		...
		if (result === "(none)") {
			...
			if (originalState) {
				if (originalState.model) {
					await smolt.setModel(originalState.model);
				}
				smolt.setThinkingLevel(originalState.thinkingLevel);
				smolt.setActiveTools(originalState.tools);
			} else {
				smolt.setActiveTools(["read", "bash", "edit", "write"]);
			}
	...
	smolt.registerShortcut(Key.ctrlShift("u"), {
	...
	smolt.on("session_start", async (_event, ctx) => {
		...
		const presetFlag = smolt.getFlag("preset");
	...
	smolt.on("turn_start", async () => {
		if (activePresetName) {
			smolt.appendEntry("preset-state", { name: activePresetName });
		}
	});
}
