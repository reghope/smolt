...
export interface PiCommandContext {
	runSmolt(command: PiCommand): void | Promise<void>;
}

const listenOption = transportOption("--listen");

export const smoltCommand = new Command<PiCommand, PiCommandContext>("smolt")
	.option(listenOption)
	...
	.action((command, context) => context.runSmolt(command));
