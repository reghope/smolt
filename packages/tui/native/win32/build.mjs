...
const temporaryDir = mkdtempSync(path.join(tmpdir(), "smolt-tui-win32-"));
...
Environment:
  SMOLT_TUI_WIN32_TOOLCHAIN=msvc|mingw
...
const requestedToolchain = process.env.SMOLT_TUI_WIN32_TOOLCHAIN;
if (requestedToolchain && requestedToolchain !== "msvc" && requestedToolchain !== "mingw") {
	throw new Error("SMOLT_TUI_WIN32_TOOLCHAIN must be either 'msvc' or 'mingw'");
}
...
if (toolchain === "msvc") {
	if (!vsDevCmd || !hasMsvc) {
		throw new Error("Microsoft C++ Build Tools not found. Install the Visual Studio C++ workload or set SMOLT_TUI_WIN32_TOOLCHAIN=mingw.");
	}
