...
describe("AgentSession concurrent prompt guard", () => {
	...
	beforeEach(async () => {
		tempDir = join(tmpdir(), `smolt-concurrent-test-${Date.now()}`);
	...
	it("should queue extension-origin steering messages while streaming", async () => {
		...
		const extensionsResult = await createTestExtensionsResult([
			(smolt) => {
				(globalThis as typeof globalThis & { testExtensionApi?: unknown }).testExtensionApi = smolt;
			},
			(smolt) => {
		...
		expect(smolt).toBeDefined();

		smolt!.sendUserMessage("Steer from extension", { deliverAs: "steer" });
