...
Smolt and also where the boundaries are.

In general Smolt is a coding agent that runs locally within the security boundary
...
Smolt treats the local user account and files writable by that account as inside
the same trust boundary as the Smolt process itself.  If an attacker can modify files
under the user's home directory, workspace, shell startup files, environment, or
Smolt configuration, they can generally influence Smolt or other local developer tools.
Reports that depend on such prior local write access are not security
vulnerabilities unless they demonstrate how Smolt grants that write access or crosses
...
- Public internet exposure of a Smolt installation
...
- Reports requiring the ability to create, modify, delete, or replace files,
  ...
  user-controlled local state on the target machine. This includes `~/.smolt`,
  `~/.smolt/agent/models.json`, workspace files, `AGENTS.md`, skills, extensions,
...
For example, a report showing that malicious contents written to a trusted Smolt
configuration file cause Smolt to execute commands, load attacker-controlled tools,
...
affected and that the issue is reachable through Smolt.  For exposed-secret reports,
