...
describe("ProjectTrustStore", () => {
	...
	it("detects trust-requiring project resources", () => {
		...
		try {
			mkdirSync(join(tempDir, ".smolt", "agent"), { recursive: true });
			...
			writeFileSync(join(tempDir, ".smolt", "settings.json"), "{}");
			expect(hasTrustRequiringProjectResources(tempDir)).toBe(true);
			rmSync(join(tempDir, ".smolt", "settings.json"), { force: true });

			mkdirSync(join(cwd, ".smolt"), { recursive: true });
			writeFileSync(join(cwd, ".smolt", "settings.json"), "{}");
