...
export interface CreateCodingAgentHarnessOptions extends Omit<AgentHarnessOptions, "toolContext" | "tools"> {
	...
	/** Path to the JSONL session file exposed to default bash commands as SMOLT_SESSION_FILE. */
	sessionFile?: string;
			...
			createCodingAgentHarnessTool(
				createBashTool<ExecutionToolContext>({
					...
					prepare: async (execution) => {
						...
						execution.env.SMOLT_SESSION_ID = metadata.id;
						execution.env.SMOLT_SESSION_FILE = sessionFile ?? "";
						execution.env.SMOLT_PROVIDER = model.provider;
						execution.env.SMOLT_MODEL = model.id;
						execution.env.SMOLT_REASONING_LEVEL = thinkingLevel;
					},
				}),
				toolContext,
				{
