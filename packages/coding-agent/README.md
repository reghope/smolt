...
Smolt also supports the llama.cpp router server. Configure it with `/login llama.cpp`, manage downloads and loaded models with `/llama`, then select a loaded model with `/model`. See [docs/llama-cpp.md](docs/llama-cpp.md) for setup and usage.

See [docs/providers.md](docs/providers.md) for other provider setup instructions.

**Custom providers & models:** Add providers via `~/.smolt/agent/models.json` if they speak a supported API (OpenAI, Anthropic, Google). For custom APIs or OAuth, use extensions. See [docs/models.md](docs/models.md) and [docs/custom-provider.md](docs/custom-provider.md).
...
See `/hotkeys` for the full list. Customize via `~/.smolt/agent/keybindings.json`. See [docs/keybindings.md](docs/keybindings.md).
...
Sessions auto-save to `~/.smolt/agent/sessions/` organized by working directory.

```bash
smolt -c                  # Continue most recent session
smolt -r                  # Browse and select from past sessions
smolt --no-session        # Ephemeral mode (don't save)
smolt --name "my task"    # Set session display name at startup
smolt --session <path|id> # Use specific session file or ID
smolt --fork <path|id>    # Fork specific session file or ID into a new session
```
...
| Location | Scope |
|----------|-------|
| `~/.smolt/agent/settings.json` | Global (all projects) |
| `.smolt/settings.json` | Project (overrides global) |
...
- `~/.smolt/agent/AGENTS.md` (global)
- Parent directories (walking up from cwd)
- Current directory

If a directory contains `AGENTS.override.md`, Smolt loads it instead of `AGENTS.md` or `CLAUDE.md` from that directory. Context files from other directories are still concatenated.
...
Replace the default system prompt with `.smolt/SYSTEM.md` (project) or `~/.smolt/agent/SYSTEM.md` (global). Append without replacing via `APPEND_SYSTEM.md`.
...
```markdown
<!-- ~/.smolt/agent/skills/my-skill/SKILL.md -->
# My Skill
...
export default function (smolt: ExtensionAPI) {
  smolt.registerTool({ name: "deploy", ... });
...
```bash
smolt install npm:@foo/smolt-tools
smolt install npm:@foo/smolt-tools@1.2.3      # pinned version
smolt install git:github.com/user/repo
smolt install git:github.com/user/repo@v1  # tag or commit
smolt install git:git@github.com:user/repo
smolt install git:git@github.com:user/repo@v1  # tag or commit
smolt install https://github.com/user/repo
smolt install https://github.com/user/repo@v1      # tag or commit
smolt install ssh://git@github.com/user/repo
smolt install ssh://git@github.com/user/repo@v1    # tag or commit
smolt remove npm:@foo/smolt-tools
smolt uninstall npm:@foo/smolt-tools          # alias for remove
smolt list
smolt update                               # update smolt only
smolt update --all                         # update smolt and packages
smolt update --extensions                  # update packages only
...
smolt update npm:@foo/smolt-tools             # update one package
smolt config                               # enable/disable extensions, skills, prompts, themes
```

Packages install to `~/.smolt/agent/git/` (git) or `~/.smolt/agent/npm/` (npm). Use `-l` for project-local installs (`.smolt/git/`, `.smolt/npm/`). Git `@ref` values are pinned tags or commits; pinned packages are skipped by `smolt update --extensions` and `smolt update --all`, so use `smolt install git:host/user/repo@new-ref` to move an existing package to a new ref. Git packages install dependencies with `npm install --omit=dev` by default, so runtime deps must be listed under `dependencies`; when `npmCommand` is configured, git packages use plain `install` for compatibility with wrappers. If you use a Node version manager and want package installs to reuse a stable npm context, set `npmCommand` in `settings.json`, for example `["mise", "exec", "node@20", "--", "npm"]`.

Create a package by adding a `smolt` key to `package.json`:

```json
{
  "name": "my-smolt-package",
...
Read the [blog post](https://mariozechner.at/posts/2025-11-30-smolt-coding-agent/) for the full rationale.
...
`smolt config` and project package commands accept `--approve`/`--no-approve` to trust or ignore project-local settings for one command. `smolt update` never prompts for project trust.
...
# High thinking level
smolt --thinking high "Solve this complex problem"
```
