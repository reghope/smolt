...
import * as _bundledSmoltAgentCore from "@smolt/agent-core";
import type { Provider } from "@smolt/ai";
import * as _bundledSmoltAiCompat from "@smolt/ai/compat";
import * as _bundledSmoltAiOauth from "@smolt/ai/oauth";
import * as _bundledSmoltAiProviders from "@smolt/ai/providers/all";
import type { KeyId } from "@smolt/tui";
import * as _bundledSmoltTui from "@smolt/tui";
...
const VIRTUAL_MODULES: Record<string, unknown> = {
	...
	"@smolt/agent-core": _bundledSmoltAgentCore,
	"@smolt/tui": _bundledSmoltTui,
	// Extensions resolve the smolt-ai root to the compat entrypoint (a strict
	// superset of the core entrypoint): existing extensions using the old
	// global API keep working at runtime until compat is removed.
	"@smolt/ai": _bundledSmoltAiCompat,
	...
	"@mariozechner/smolt-ai/compat": _bundledSmoltAiCompat,
	"@mariozechner/smolt-ai/oauth": _bundledSmoltAiOauth,
	"@mariozechner/smolt-ai/providers/all": _bundledSmoltAiProviders,
	"@mariozechner/smolt-coding-agent": _bundledSmoltCodingAgent,
};
...
function getAliases(): Record<string, string> {
	...
	const smoltCodingAgentEntry = packageIndex;
	const smoltAgentCoreEntry = resolveWorkspaceOrImport("agent/dist/index.js", "@smolt/agent-core");
	const smoltTuiEntry = resolveWorkspaceOrImport("tui/dist/index.js", "@smolt/tui");
	// Extensions resolve the smolt-ai root to the compat entrypoint (a strict
	// superset of the core entrypoint): existing extensions using the old
	// global API keep working at runtime until compat is removed.
	...
	_aliases = {
		...
		"@smolt/ai/compat": smoltAiCompatEntry,
		"@smolt/ai/oauth": smoltAiOauthEntry,
		"@smolt/ai": smoltAiCompatEntry,
		"@mariozechner/smolt-coding-agent": smoltCodingAgentEntry,
		"@mariozechner/smolt-agent-core": smoltAgentCoreEntry,
		"@mariozechner/smolt-tui": smoltTuiEntry,
		"@mariozechner/smolt-ai/providers/all": smoltAiProvidersEntry,
		"@mariozechner/smolt-ai/compat": smoltAiCompatEntry,
		"@mariozechner/smolt-ai/oauth": smoltAiOauthEntry,
		"@mariozechner/smolt-ai": smoltAiCompatEntry,
...
function createExtensionAPI(
	...
	const api = {
		...
		abortResponse(reason, options): boolean {
			assertActive();
			return runtime.abortResponse(reason, options);
		},
 ...
 *
 * Checks for:
 * 1. package.json with "smolt.extensions" field -> returns declared paths
 * 2. index.ts or index.js -> returns the index file
 *
 * Returns resolved paths or null if no entry points found.
 */
function resolveExtensionEntries(dir: string): string[] | null {
	// Check for package.json with "smolt" field first
	const packageJsonPath = path.join(dir, "package.json");
	if (fs.existsSync(packageJsonPath)) {
		const manifest = readSmoltManifest(packageJsonPath);
 ...
 * 3. Subdirectory with package.json: `extensions/* /package.json` with "smolt" field → load what it declares
 *
...
export async function discoverAndLoadExtensions(
	...
	for (const p of configuredPaths) {
		...
		if (fs.existsSync(resolved) && fs.statSync(resolved).isDirectory()) {
			// Check for package.json with smolt manifest or index.ts
