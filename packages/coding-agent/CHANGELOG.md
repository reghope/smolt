...
- Added a "Local web server" switch to the desktop's settings: the app in a browser, served from the running desktop process — same agent, same chats — on localhost and the machine's Tailscale address (port 7332, HTTPS on 7333 with a self-signed certificate made by openssl on first use). It replaces the standalone `.smolt/webhost` host, which ran a second copy of the main process. Every window on the app shows the same chat: messages sent from one stream into the others, and a chat opened in one is followed by the rest.

- Added `tools`, a built-in extension that gives smolt's tools leaner habits: every tool result is cut to a token budget at the boundary where it enters history (10,000 tokens by default; head and tail kept, the middle dropped behind `…N tokens truncated…`), prompt lines about searching with `rg`, parallel reads and not re-reading after edits, and the read tool told that when you already know which part of the file you need, only read that part. A shell mode (`~/.smolt/agent/tools.json`, `"read": "shell"`) removes the read tool altogether — files are read with `sed -n`, `nl` and `rg`, and only images keep a tool, `view_image`. An opt-in first look caps unranged reads at N lines; it measured well but is off by default. Switchable in settings like any other built-in.
- Added per-extension switches: every extension the install knows about is listed in the desktop's settings with a toggle and a line saying what it does, and a switched-off extension is skipped before it loads rather than loaded and hidden. Ids are stable across machines (a built-in's name, a file extension's own file), machine-caused failures aggregate everywhere while project-caused ones stay in their project, and the choice is written to `disabledExtensions` in global settings.
- Added cues: house notes that enter the system prompt only once their subject comes up, instead of being paid for on every request of every session. A cue arms on a trigger phrase, optionally alongside a second phrase, and never arms when the prompt already settles the question it answers. Ships with one — a new web app with no stack named starts on Vite with React Router — and reads more from `~/.smolt/agent/cues/*.md`, where a file replaces a built-in of the same name. `/cues` lists them and which are armed.
- Added a "Delete all local data" button to the desktop's settings, behind a confirmation that names what goes: every chat, the curated memory, agent-written skills, cues, and the session index and tool telemetry. Provider credentials, pooled keys and preferences are deliberately spared, the agents are stopped first so Windows cannot hold the database open, and the app restarts on an empty history.
- Added list selection to the desktop sidebar: Ctrl+A takes every chat shown, Ctrl-click picks chats out one at a time, Shift-click takes the range between, and right-clicking inside a selection offers to delete the lot. Right-clicking outside one clears it first, so a destructive menu never acts on chats the reader has stopped pointing at.
- Added a composer history that survives: Up and Down walk previously sent prompts and commands with a `History n/total` marker above the input, and the list now persists across chats and restarts instead of starting empty each launch.
- Added a Telegram heading to the desktop sidebar, above the days, so the chat the dedicated Telegram host writes into is where phone messages are read rather than sinking into the day it started.

- Added `/research <subject>`: a team of investigators on the battletest foundation (background sessions, live roster, diaries, clearance loop, synthesis) that stops at nothing short of the answer. Researchers get `search` and `fetch` tools (dependency-free; text/html/json/links/scripts/headers), a `browse` tool that also reads rendered text, the live DOM, and every network request the page makes, and a `relaunch` action that restarts the browser as a visible window when a site refuses the headless one; then the shell for repositories, packages, bundles, source maps, and archives. Findings carry a confidence, a kind, and their sources; the team works a question map kept from wayfinder (sharp sub-questions, blocking edges, claims, a computed frontier), and the supervisor dispatches further waves (`research` action `continue`) while questions remain takeable. Runs live in the project's research store under `~/.smolt/projects/<project>/research/`; the report leads with the answer.
- Added `headed`, `userAgent`, and `captureNetwork` options to the battletest CDP driver (`requests()` drains captured requests), and the child-session spawner and model-override resolution battletest and research now share (`battletest/spawn.ts`, `battletest/parse.ts`).
- Added semantic recall, on by default: past sessions are embedded with a small model that runs on this machine (`Xenova/bge-small-en-v1.5`, downloaded once into `~/.smolt/agent/models`) and `session_search` fuses meaning with full-text; long messages are embedded in chunks so their tails stay findable, and unchanged session files are skipped without being read. It is a built-in extension of its own, `semantic-recall`, switchable in settings without losing memory, skills, or lexical search; `/embeddings` reports the model, the index, and the last backfill. The desktop app hands its bundled transformers.js to the agents it spawns. The CLI package does not carry the runtime, so there it runs when `@huggingface/transformers` is installed beside smolt or named in `SMOLT_EMBEDDINGS_MODULE`; `~/.smolt/agent/embeddings.json` can point either at an OpenAI-compatible embedding server instead.
...
- Added an "Advisor model" setting to the desktop's Model page: the advisor follows the chat model by default, or reviews with a model picked here, written to `model` in the user-level `advisor.json` and read again before each review so it applies to the running chat. RPC gained `get_advisor_settings` and `set_advisor_model` (a "provider/model-id" the registry knows, or empty to follow the chat).
...
- Added `/review`: verified code review as a built-in extension. Bare it reviews the pending work, or it takes a PR number, branch, commit range, or path. Every finding must name a concrete failure scenario or it is dropped, and a re-review of the same target re-verifies what earlier reviews left standing instead of repeating it. `/review setup` wires the repo so each opened or updated pull request gets one self-updating review comment: it offers only providers smolt already holds an API key for (subscription logins are named and excluded, since GitHub Actions cannot use them), names the repository secret after the local credential, and offers to upload that key with `gh secret set` over stdin so the credential is never defined twice or exposed in a process argument list. `/review setup --remove` deletes the workflow.
...
- An advisory note now reads as a message addressed to the agent, which answers it and acts on it if it is right. Mid-run nothing changes: the note steers the turn already running. When the agent is idle a note of any severity starts a turn, where before only a blocker did and a nit or concern sat unread until the reader typed again. At most two advisor-started turns run in a row without user input, and an aborted chat starts no further ones until the reader speaks.
- Spend is reported in tokens only, everywhere: the battletest and research rosters, footers, `wait` usage lines, `performance.json` and `form.jsonl` drop the dollar figure (`$0.42`-style labels and the `cost` fields are gone), the goal's completion prompt speaks of tokens spent instead of cost, and a child run's spend carried back to the session stats as tool-result usage no longer carries a dollar amount.

- Cut what a research, battletest, or subagent run spends. Measured before: a three-researcher wave re-read 19-20k tokens on each of 35-60 turns per researcher (2.7 million for the wave), a phone tester's screenshot was 1125x2436 pixels, and a subagent thread carried six thousand tokens of skills catalogue and AGENTS.md on every turn at the parent's thinking level. Now: research's `browse` returns a screenshot only when asked (`screenshot`), pages are capped at 8,000 characters by default, a researcher keeps three recent results whole at 4,000 tokens, runs stop at three waves; testers keep four recent screenshots, shed in batches of four; every child trims the tool calls behind shed results (the prose in notebook and testlog filings, which never left on its own); screenshots are captured at CSS-pixel size and at most 1024 wide; subagent threads run on the lean child loader (skills off, context files only for agents that can edit, results budgeted and shed), default to medium thinking (the built-in explorer to low), honour `agents.defaultSubagentModel` and `agents.defaultSubagentThinking`, and `read` returns a transcript's tail rather than the whole; the supervising session's `wait` defaults to five minutes and returns early on news (a finding, a ticket, a closed question, a finished child) once ninety seconds have passed, so a quiet stretch costs no turn; and the `research` and `battletest` tool descriptions, which ride on every turn of every session, are shorter (about 1,500 and 1,100 tokens with their parameters, from 1,600 and 1,300). Every child also drops earlier turns' thinking before each request — providers that take reasoning back in history (opencode-go's GLM among them) were re-reading every past turn's thinking as input — the researcher brief is a fifth shorter with every rule kept, and a supervisor-picked research team is capped at three angles. `agents.persistChildSessions` is now read from settings.json (it was read from a context field that did not exist).

- The advisor now costs a small fraction of what it did: a review is one request of about a thousand tokens, where one review used to reach fifteen thousand. It reviews an in-progress run every six steps (`reviewEvery` in `advisor.json`) and once when the run settles, where it used to review after every model step, and not at all over a stretch of transcript with no tool calls, tool results, shell commands or failed steps, which is folded into the next update that carries work. It holds no investigative tools unless a `WATCHDOG.yml` roster grants them (`tools:`), so it answers `ok` or calls `advise` and the review ends there, never asked again once it has spoken; a roster with tools gets at most two more calls. A mid-run review is also skipped when the stretch since the last one holds fewer than three tool calls, tool results or shell commands and no error, so a couple of calls wait and join the next update instead of buying a request of their own. The request thinks at the minimal level, raised by `thinking` in `advisor.json` (`"off"` through `"max"`), and its reply is capped at 700 tokens. An update is at most 3,500 characters with the head and tail kept, and within it thinking, tool arguments and tool results are each cut to 240 characters (an error result to 600); afterwards the exchange collapses to the update cut to 240 characters and the closing words cut to 200, its tool calls and results dropped, and only that one exchange is carried forward. The system prompt is shorter and mentions investigation only when tools are granted. `ModelRegistry` gained `completeSimple`, so an extension can send a request with provider-neutral options.

- Removed the taste extension (the self-arming design doctrine and its finish gate). A prompt with any attached image armed it as design work, putting ~23K tokens of doctrine into every request of the session; a coding agent attaches screenshots for far more than design work. The doctrine files, `taste_review` tool, trigger, gate, and the analyst's attribution and fix entries for it are gone with it.

- Changed where the agent keeps what it writes about a project: battletest runs, research runs, reviews and wayfinder maps now live under `~/.smolt/projects/<project>/`, not in the project's own `.smolt/` directory. They were never source, and a single battletest run put hundreds of generated ticket files between the reader and the handful of files they had actually changed. Each project directory is named after the project folder and records the path it belongs to, so two checkouts called `api` keep their own records. On first use anything already under `.smolt/<store>/` is carried across and the old directory removed, which shows up as an ordinary deletion in git. What the reader writes — settings, prompts, skills, subagents — stays in the project untouched.
...
- Stopping a chat once now stops it. Messages queued behind the turn — a research or battletest kickoff brief delivered as a follow-up, and the narration nudge queued behind it — each started a fresh turn the moment the stopped one ended, so stopping a run took three presses. `abort()` now drops everything queued behind the turn; `clearQueue()` hands back only the reader's own queued words, never an extension's brief, and the desktop returns those to the composer when the stop button is pressed.

- Fixed the advisor restarting work the reader just stopped: pressing stop killed the request in flight, but the advisor's settled review then saw the aborted partial work, called it a blocker, and `triggerTurn` started a new turn - so stopping took two or three presses and the chat kept reviving (resuming runs, re-entering wait loops). After an abort the advisor now holds its review until a new turn actually starts, checked both by an abort flag and by the transcript ending in an aborted assistant message, since the fire-and-forget abort emit can lose the race against `turn_end`/`agent_settled`.
- Fixed OpenCode requests going out without the `x-opencode-session` header on paths that bypass the agent loop's header transform: session headers now apply in `ModelRuntime.prepareRequest`, so `modelRegistry.complete()` calls (advisor, research and battletest label requests) carry the same per-conversation id as ordinary streaming requests.
- Fixed the web-stack cue sleeping through most of the web. Its trigger list now covers the verbs that start something without saying "build" (write, put together, clone, replicate, recreate, rebuild) and its subject list the web you read as well as the web you use (blog, portfolio, docs portal, storefront, microsite, web page). A list entry can now also be a pattern, written `/like this/`, and the cue uses one to catch the prompt that names no web word at all and instead holds up a page — "replicate what https://docs.example.com has going on" — which previously started a docs site on whatever stack the model reached for first. A link on its own still arms nothing.
- Fixed hindsight learning from the wrong things: stored arguments are redacted before they are written (they reach the system prompt, so a credential must never reach a row, and rows written before this are scrubbed once on migration), a nonzero exit that is a command's normal answer — grep finding nothing, diff finding differences — is recorded as the success it is instead of polluting the exit-nonzero class, notes are scoped to the project they came from unless the failure is machine-caused, and a timeout now quotes how long successful runs of that call actually took.
- Fixed auto thinking announcing itself on every new chat: selecting the entry no longer raises a notification, since new sessions apply the default through that same path and the mode is already shown in the composer. `/auto-thinking on` still answers.
- Fixed the command palette reopening the instant it was dismissed: clicking away hid it and then refocused the composer terminal-style, and the focus handler read that as a request to bring it back.
