...
import { fauxAssistantMessage, fauxToolCall, type Model } from "@smolt/ai";
...
describe("AgentSession prompt characterization", () => {
	...
	it("expands skill commands before sending the prompt", async () => {
		const tempDir = join(tmpdir(), `smolt-skill-${Date.now()}-${Math.random().toString(36).slice(2)}`);
	...
	it("throws when prompted during manual compaction", async () => {
		...
		const harness = await createHarness({
			...
			extensionFactories: [
				(smolt) => {
					smolt.on("session_before_compact", async (event) => {
