...
#   ln -s "$PWD/scripts/auto-smolt.sh" "$HOME/.local/bin/smolt"
#
# ~/.local/bin must appear before the stable smolt installation on PATH.

# Resolve this script through symlinks so repo_dir points at the development
# checkout rather than the directory containing the `smolt` symlink.
...
if [[ "$use_stable" == true ]]; then
	if ! stable_smolt="$(find_stable_smolt)"; then
		echo "error: could not find a stable smolt executable after the auto-smolt wrapper on PATH" >&2
		exit 1
	fi
	exec "$stable_smolt" ${args[@]+"${args[@]}"}
fi

dev_smolt="$repo_dir/packages/coding-agent/dist/cli.js"
if [[ ! -x "$dev_smolt" ]]; then
	echo "error: development smolt build not found; run \`npm run build\` in $repo_dir" >&2
	exit 1
fi
