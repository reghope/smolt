...
export default function (smolt: ExtensionAPI) {
	...
	smolt.on("session_start", async (event, ctx) => {
		ctx.ui.setTitle(event.reason === "new" ? "smolt RPC Demo (new session)" : "smolt RPC Demo");
	...
	smolt.registerCommand("rpc-input", {
	...
	smolt.registerCommand("rpc-editor", {
	...
	smolt.registerCommand("rpc-prefill", {
