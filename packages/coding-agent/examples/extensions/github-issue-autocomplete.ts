...
async function resolveGitHubRepo(smolt: ExtensionAPI, cwd: string): Promise<RepoResolution> {
	const result = await smolt.exec("git", ["remote", "-v"], { cwd, timeout: 5_000 });
	if (result.code !== 0) {
...
export default function (smolt: ExtensionAPI): void {
	smolt.on("session_start", async (_event, ctx) => {
		const resolvedRepo = await resolveGitHubRepo(smolt, ctx.cwd);
		if (!resolvedRepo.ok) {
