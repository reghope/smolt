 ...
 *
 * Runs smolt's built-in tools inside a local Gondolin micro-VM. The host working
 ...
 * Usage:
 *   cd /path/to/project
 *   smolt -e /path/to/smolt/packages/coding-agent/examples/extensions/gondolin
 *
 * Requirements:
...
export default function (smolt: ExtensionAPI) {
	...
	async function startVm(ctx?: ExtensionContext): Promise<VM> {
		...
		const created = await VM.create({
			sessionLabel: `smolt ${path.basename(localCwd)}`,
			vfs: {
				mounts: {
	...
	smolt.registerCommand("gondolin", {
	...
	smolt.on("user_bash", async (_event, ctx) => {
		const activeVm = await ensureVm(ctx);
