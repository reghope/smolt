...
- **session_search** — search across all prior sessions with four calling shapes: *discovery* (query → deduped sessions, top result fully hydrated with bookends and a ±5 message window), *scroll* (anchored ±N window inside a session), *read* (whole session, bounded for large ones), and *browse* (recent sessions). Every shape returns actual messages, not summaries. Full-text (SQLite FTS5, plain-scan fallback) fused with search by meaning: a small embedding model runs on the machine — weights downloaded once, no API bill, no text leaving the machine — so a query that shares no words with the original conversation can still find it. Listed in settings as its own extension, `semantic-recall`, so it can be switched off on its own; `~/.smolt/agent/embeddings.json` can point it at an embedding server instead.

A periodic nudge reminds the agent to persist anything durable it learned during long sessions. `/hindsight` shows what the telemetry has observed; `/skills` shows which authored skills anything has actually loaded.
...
- CI installs with `npm ci`, and a scheduled workflow runs `npm audit --omit=dev` plus `npm audit signatures --omit=dev`.
