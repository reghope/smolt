...
Example extensions for smolt-coding-agent.

## Usage

```bash
# Load an extension with --extension flag
smolt --extension examples/extensions/permission-gate.ts

# Or copy to extensions directory for auto-discovery
cp permission-gate.ts ~/.smolt/agent/extensions/
```
...
| `public-apis.ts` | `public_apis` tool + `/public-apis` command: searches the public-apis/public-apis catalog (cached README) for real free HTTP APIs |
...
| `send-user-message.ts` | Demonstrates `smolt.sendUserMessage()` for sending user messages from extensions |
...
| Extension | Description |
|-----------|-------------|
| `mac-system-theme.ts` | Syncs Smolt theme with macOS dark/light mode |
...
| `event-bus.ts` | Inter-extension communication via `smolt.events` |
...
| `custom-provider-gitlab-duo/` | GitLab Duo provider using smolt-ai's built-in Anthropic/OpenAI streaming via proxy |
