...
import { Markdown, type MarkdownTheme } from "@smolt/tui";
...
import { formatVersionCheckError, getLatestSmoltRelease, isNewerPackageVersion } from "./utils/version-check.ts";
import {
...
function getActiveManagedInstallRoot(): string | undefined {
	...
	// source checkout or another Smolt installation launched from managed Smolt as managed.
	...
	try {
		...
		if (marker.kind !== "smolt-managed-install" || marker.schemaVersion !== 1 || marker.layout !== "releases-v1") {
			throw new Error();
		}
	} catch {
...
async function fetchInstallerArtifact(url: string, label: string): Promise<string> {
	const response = await fetch(url, { headers: { "User-Agent": getSmoltUserAgent(VERSION) } });
	if (!response.ok) {
...
function verifyManagedRelease(releaseDir: string, expectedVersion: string): void {
	...
	if (result.error || result.status !== 0) {
		...
		throw new Error(`Could not verify managed Smolt ${expectedVersion}: ${reason}`);
	}
	const installedVersion = result.stdout.trim();
	if (installedVersion !== expectedVersion) {
		throw new Error(`Managed Smolt smoke test returned version ${installedVersion}; expected ${expectedVersion}.`);
	}
}
...
async function runManagedSelfUpdate(managedRoot: string, version: string): Promise<void> {
	...
	try {
		...
		if (error instanceof Error && "code" in error && error.code === "ELOCKED") {
			throw new Error("Another managed Smolt update is already running.");
		}
		throw error;
	}
...
Update smolt, installed packages, or model catalogs.

Options:
  --self                  Update smolt only (default when no target is given)
  --extensions            Update installed packages only
  --models                Refresh model catalogs only
  --all                   Update smolt and installed packages
  --extension <source>    Update one package only
  -a, --approve           Trust project-local files for this command
  -na, --no-approve       Ignore project-local files for this command
  --force                 Reinstall smolt even if the current version is latest

Short forms:
  ${APP_NAME} update                Update smolt only
  ${APP_NAME} update --all          Update smolt and all extensions
  ${APP_NAME} update --models       Refresh model catalogs only
  ${APP_NAME} update <source>       Update one package
  ${APP_NAME} update smolt             Update smolt only (self works as alias to smolt)
`);
			return;
...
async function getSelfUpdatePlan(force: boolean): Promise<SelfUpdatePlan> {
	let latestRelease: Awaited<ReturnType<typeof getLatestSmoltRelease>>;
	try {
		latestRelease = await getLatestSmoltRelease(VERSION, { retry: true });
	} catch (error: unknown) {
