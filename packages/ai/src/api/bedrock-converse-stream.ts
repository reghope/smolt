...
export const stream: StreamFunction<"bedrock-converse-stream", BedrockOptions> = (
	...
	(async () => {
		...
		// A profile explicitly configured through smolt's auth flow (the `profile`
...
function addCustomHeadersMiddleware(client: BedrockRuntimeClient, headers: Record<string, string>): void {
	...
	client.middlewareStack.add(middleware, { step: "build", name: "smolt-ai-custom-headers", priority: "low" });
}
...
function addResponseHeadersMiddleware(
	...
	client.middlewareStack.add(middleware, { step: "deserialize", name: "smolt-ai-response-headers" });
}
 ...
 * Defaults to "short" and uses SMOLT_CACHE_RETENTION for backward compatibility.
 */
