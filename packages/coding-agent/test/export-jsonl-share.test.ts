...
import type { AssistantMessage, ToolResultMessage } from "@smolt/ai/compat";
...
describe("JSONL share export", () => {
	...
	it("adds presentation data without changing conversation IDs or links", async () => {
		const tempDir = mkdtempSync(join(tmpdir(), "smolt-jsonl-share-"));
		...
		try {
			...
			expect(normalRecords.some((record) => record.type === "custom" && record.customType === "smolt.share")).toBe(
				false,
			);
