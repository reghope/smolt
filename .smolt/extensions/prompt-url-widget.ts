...
async function fetchAdvisoryMetadata(smolt: ExtensionAPI, cwd: string, target: string): Promise<GhMetadata | undefined> {
	const advisoryRef = parseAdvisoryUrl(target) ?? (await readAdvisoryRefFromDraft(cwd, target));
	if (!advisoryRef) return undefined;

	try {
		const result = await smolt.exec("gh", [
			"api",
...
async function fetchGhMetadata(
	...
	if (kind === "advisory") {
		return fetchAdvisoryMetadata(smolt, cwd, target);
	}
	...
	try {
		const result = await smolt.exec("gh", args);
...
export default function promptUrlWidgetExtension(smolt: ExtensionAPI) {
	...
	const applySessionName = (ctx: ExtensionContext, match: PromptMatch, metadata?: GhMetadata) => {
		...
		const currentName = smolt.getSessionName()?.trim();
		if (!currentName) {
			smolt.setSessionName(desiredName);
			return;
		}
	...
	const updatePromptContext = (ctx: ExtensionContext, match: PromptMatch) => {
		...
		void fetchGhMetadata(smolt, match.kind, match.target, ctx.cwd).then((meta) => {
	...
	smolt.on("session_switch", async (_event, ctx) => {
