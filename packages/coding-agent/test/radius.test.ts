...
import { InMemoryModelsStore } from "@smolt/ai";
...
beforeEach(() => {
	allowNetwork();
	tempDir = join(tmpdir(), `smolt-test-radius-${Date.now()}-${Math.random().toString(36).slice(2)}`);
...
describe("Radius provider", () => {
	it("restores the legacy credential catalog without network access", async () => {
		...
		expect(model).toMatchObject({ api: "smolt-messages", baseUrl: "https://radius.example.com/v1" });
