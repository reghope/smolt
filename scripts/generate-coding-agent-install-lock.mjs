...
const internalPackagePrefix = "@smolt/";
const installPackageName = "@smolt/coding-agent-install";
...
function createInstallerPackageJson(codingAgentPackage) {
	const packageJson = {
		...
		private: true,
		description: "Lockfile root used by the Smolt installer and updater.",
		dependencies: {
...
function validateGeneratedFiles(installerPackageJson, installLock, internalNames) {
	...
	for (const [lockPath, entry] of Object.entries(installLock.packages)) {
		...
		if ((packageName?.startsWith(internalPackagePrefix) || packageName === "smolt") && entry.version !== installerPackageJson.version) {
