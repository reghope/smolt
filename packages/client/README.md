...
```ts
import { PiClient } from "@smolt/client";
import { createUnixTransportFactory } from "@smolt/client/unix";
...
The `@smolt/client` root remains transport- and runtime-neutral. Importing the Node-compatible transport requires the explicit `@smolt/client/unix` subpath.
