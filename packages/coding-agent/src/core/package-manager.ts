...
import { type PiManifest, readSmoltManifest } from "./smolt-manifest.ts";
...
function isOfflineModeEnabled(): boolean {
	const value = process.env.SMOLT_OFFLINE;
...
type SkillDiscoveryMode = "smolt" | "agents";
	...
	try {
		...
		for (const entry of dirEntries) {
			...
			const shouldIncludeMarkdownFile =
				...
				!ig.ignores(relPath) &&
				((mode === "smolt" && dir === root) || (mode === "agents" && dir !== root));
...
function collectResourceFiles(dir: string, resourceType: ResourceType): string[] {
	if (resourceType === "skills") {
		return collectSkillEntries(dir, "smolt");
	}
...
export class DefaultPackageManager implements PackageManager {
	...
	private getNpmInstallArgs(specs: string[], installRoot: string): string[] {
		...
		// Extension packages run inside smolt and resolve smolt APIs through loader aliases/virtual modules.
		// Disable peer dependency resolution for managed installs (npm's --legacy-peer-deps, and
		// equivalent bun/pnpm settings) so package managers do not install or solve host-provided
		// @smolt/* peers. Stale auto-installed smolt peers can otherwise block updates.
	...
	private getGitUpdateMarkerPath(targetDir: string): string {
		return join(dirname(targetDir), `.${basename(targetDir)}.smolt-update-incomplete`);
	}
	...
	private ensureNpmProject(installRoot: string): void {
		...
		if (!existsSync(packageJsonPath)) {
			const pkgJson = { name: "smolt-extensions", private: true };
	...
	private collectPackageResources(
		...
		const manifest = readSmoltManifest(join(packageRoot, "package.json"));
		if (manifest) {
			for (const resourceType of RESOURCE_TYPES) {
	...
	private addAutoDiscoveredResources(
		...
		if (projectTrusted) {
			...
			// Project skills from .smolt/
			addResources(
				"skills",
				collectAutoSkillEntries(projectDirs.skills, "smolt"),
				projectMetadata,
		...
		// User extensions from ~/.smolt/agent/
		addResources(
			"extensions",
		...
		// User skills from ~/.smolt/agent/
		addResources(
			"skills",
			collectAutoSkillEntries(userDirs.skills, "smolt"),
			userMetadata,
