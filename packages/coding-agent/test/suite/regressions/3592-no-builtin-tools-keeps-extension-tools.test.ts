...
describe("regression #3592: no-builtin-tools keeps extension tools enabled", () => {
	...
	beforeEach(() => {
		tempDir = join(tmpdir(), `smolt-no-builtin-tools-${Date.now()}-${Math.random().toString(36).slice(2)}`);
	...
	it("keeps extension tools active when built-in defaults are disabled", async () => {
		...
		expect(
			session
				...
				.map((tool) => tool.name)
				.sort(),
		).toEqual(["bash", "command", "dynamic_tool", "edit", "find", "grep", "ls", "powershell", "read", "write"]);
