...
import { type AutocompleteProvider, CombinedAutocompleteProvider } from "@smolt/tui";
...
describe("InteractiveMode.showLoadedResources", () => {
	...
	function createExtensionFixtures(): ExtensionFixture[] {
		return [
			{
				path: "/tmp/project/.smolt/extensions/answer.ts",
				sourceInfo: createSourceInfo("/tmp/project/.smolt/extensions/answer.ts", {
					source: "local",
					scope: "project",
					origin: "top-level",
					baseDir: "/tmp/project/.smolt/extensions",
				}),
			},
			{
				path: "/tmp/project/.smolt/extensions/local-index/index.ts",
				sourceInfo: createSourceInfo("/tmp/project/.smolt/extensions/local-index/index.ts", {
					source: "local",
					scope: "project",
					origin: "top-level",
					baseDir: "/tmp/project/.smolt/extensions",
			...
			},
			{
				...
				sourceInfo: createSourceInfo(
					"/tmp/project/.smolt/git/github.com/HazAT/smolt-interactive-subagents/extensions/index.ts",
					{
						source: "git:github.com/HazAT/smolt-interactive-subagents",
						scope: "project",
						origin: "package",
						baseDir: "/tmp/project/.smolt/git/github.com/HazAT/smolt-interactive-subagents",
					},
				),
			},
			{
				path: "/tmp/project/.smolt/git/github.com/HazAT/smolt-interactive-subagents/extensions/subagents/index.ts",
				sourceInfo: createSourceInfo(
					"/tmp/project/.smolt/git/github.com/HazAT/smolt-interactive-subagents/extensions/subagents/index.ts",
					{
						source: "git:github.com/HazAT/smolt-interactive-subagents",
						scope: "project",
						origin: "package",
...
"[Extensions]
  @scope/smolt-scoped, answer.ts, cli-extension.ts, HazAT/smolt-interactive-subagents, HazAT/smolt-interactive-subagents:subagents, local-index, smolt-markdown-preview, user-index"`);
	});
...
"[Extensions]
  main.ts"`);
	...
	test("package extensions still strip index.ts correctly (regression guard)", () => {
		...
		expect(normalizeRenderedOutput(fakeThis.loadedResourcesContainer)).toMatchInlineSnapshot(`
"[Extensions]
  smolt-markdown-preview"`);
	});

	test("labels npm sibling extensions relative to the declaring package", () => {
		const extensions: ExtensionFixture[] = [
			{
				path: "/tmp/project/.smolt/npm/node_modules/primary-package/index.ts",
				sourceInfo: createSourceInfo("/tmp/project/.smolt/npm/node_modules/primary-package/index.ts", {
					source: "npm:primary-package",
					scope: "project",
					origin: "package",
					baseDir: "/tmp/project/.smolt/npm/node_modules/primary-package",
				}),
			},
			{
				path: "/tmp/project/.smolt/npm/node_modules/sibling-package/index.ts",
				sourceInfo: createSourceInfo("/tmp/project/.smolt/npm/node_modules/sibling-package/index.ts", {
					source: "npm:primary-package",
					scope: "project",
					origin: "package",
					baseDir: "/tmp/project/.smolt/npm/node_modules/primary-package",
...
"[Extensions]
  primary-package, primary-package:../sibling-package"`);
	...
	test("labels Windows npm sibling extensions relative to the declaring package", () => {
		const primaryPath = "C:\\Users\\me\\.smolt\\agent\\npm\\node_modules\\primary-package\\index.ts";
		const siblingPath = "C:\\Users\\me\\.smolt\\agent\\npm\\node_modules\\sibling-package\\index.ts";
		const baseDir = "C:\\Users\\me\\.smolt\\agent\\npm\\node_modules\\primary-package";
		const extensions: ExtensionFixture[] = [
			{
				path: primaryPath,
...
"[Extensions]
  project
    /tmp/project/.smolt/extensions/answer.ts
    /tmp/project/.smolt/extensions/local-index
    git:github.com/HazAT/smolt-interactive-subagents
      extensions
      extensions/subagents
    npm:@scope/smolt-scoped
      extensions
    npm:smolt-markdown-preview
      extensions
  user
  ...
  path
	...
	test("shows context paths relative to cwd while preserving full external paths", () => {
		const home = homedir();
		const cwd = path.join(home, "Development", "smolt-mono");
		const fakeThis = createShowLoadedResourcesThis({
			quietStartup: false,
			cwd,
		...
		expect(output).toContain("~/.smolt/agent/AGENTS.md, AGENTS.md");
	...
	test("shows system prompt context paths before project context files", () => {
		...
		const fakeThis = createShowLoadedResourcesThis({
			quietStartup: false,
			cwd,
			systemPromptSource: { path: path.join(cwd, ".smolt", "SYSTEM.md") },
			appendSystemPromptSources: [{ path: path.join(cwd, ".smolt", "APPEND_SYSTEM.md") }],
		...
		const output = renderAll(fakeThis.loadedResourcesContainer).replace(/\\/g, "/");
		expect(output).toContain("[Context]");
		expect(output).toContain(".smolt/SYSTEM.md, .smolt/APPEND_SYSTEM.md, AGENTS.md");
	});
	...
	test("shows full context paths when expanded", () => {
		...
		const output = renderAll(fakeThis.loadedResourcesContainer).replace(/\\/g, "/");
		expect(output).toContain("[Context]");
		expect(output).toContain("~/.smolt/agent/AGENTS.md");
		expect(output).toContain("~/Development/smolt-mono/AGENTS.md");
		expect(output).not.toContain("~/.smolt/agent/AGENTS.md, AGENTS.md");
	});
